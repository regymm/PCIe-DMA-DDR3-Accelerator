...
## 1.Motivation
This document discusses how project documentation written in N4L can be converted into human readable text in markdown format. Sometimes, it may be required explicitly that the project documentation should be delivered in markdown format. Also, the manager may ask the employees to provide a markdown documentation for the implemented project with no special-format documentation as in N4L.

Thus, this article will help you to convert your N4L documentation into human readable markdown file. That said, this process is pretty simple where it is just the reverse of what you have done when you documented the project in N4L.

The final output that you should get after following the instructions in this article is a project documentation in markdown format out of your original documentation in N4L.

## 2.The Conversion Process

This is the core section of the document, where we discuss the steps of converting N4L documentation for a certain project into markdown format. For that to be done, one has to follow some steps.
...
Next, you have to retrieve the sections present at your N4L file. The sections may represent a separate topic in your project or discussing a certain IP/function in the project. Thus, whenever you find an N4L section title, it should be mapped to a sub-header in the corresponding markdown file.
...
Now, the next step is to extract the notes themselves from the N4L file. The notes either represent relations between two nodes thus, having an N4L arrow between them or having normal English text.

For the normal English text, you just put it as it is in your new markdown file.

For the N4L lines containing arrows, you should have known that relations between objects/events are represented with relations (arrows) in N4L. Now the question is how to convert these N4L notes into readable markdown text. As was discussed in N4L4Doc, N4L arrows are categorized into four types. For the conversion process you would have to:
 ...
 - Replace the arrow `e.g. (different!)` with its English meaning ` is nothing like `.
...
object1 (relation1) object2
    "   (relation2) object3
```
For converting such lines, you can simply replace the ditto with the last appearing source node in the previous line
This can be interpreted as:
```Text
object1 (relation1) object2
object1 (relation2) object3
```
...
------------------------------------------------------------------------
## 3.Real Case Example
Revisiting the example provided in the N4L4Doc article `eayolo.n4l`, it would be useful to demonstrate through it how N4L documentation can be converted into readable markdown text.
...
You may notice that the final markdown file may consist of sparse lines rather than consisting of informative paragraphs as documentations have conventionally. So, to improve such a file, you would take this intermediate file and enhance its sections separately. In other words, you can read the plain text lines under each section and convert them into descriptive paragraphs suiting your requirements. Nonetheless, this process should be easy as it can be considered simply as paraphrasing rather than creating a documentation from scratch.
